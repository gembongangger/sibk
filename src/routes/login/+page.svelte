		...
		<div class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl shadow-primary-900/10">
			<div class="grid md:grid-cols-2">
				...
				<section class="flex flex-col justify-center p-8 sm:p-10">
					...
					{:else}
						<div
							class="mt-5 flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-4 py-8 text-sm text-slate-500"
							role="status"
						>
							<Loader2 size={16} class="animate-spin" />
							Menyiapkan formulir login&hellip;
						</div>
					{/if}
